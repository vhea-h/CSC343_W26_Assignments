...
CREATE VIEW HighestCategories AS
    SELECT 
        mcs.month, 
        mcs.category AS highest_category, 
        mcs.sales_val AS highest_sales_val
    FROM MonthCategorySales mcs
    JOIN HighestSalesPerMonth h
...
CREATE VIEW LowestCategories AS
    SELECT 
        mcs.month, 
        mcs.category AS lowest_category, 
        mcs.sales_val AS lowest_sales_val
    FROM MonthCategorySales mcs
    JOIN LowestSalesPerMonth l
    ON mcs.month = l.month
    AND mcs.sales_val = l.lowest_sales_val;
...
INSERT INTO q4
    SELECT 
        h.month AS month,
        h.highest_category AS highest_category,
        h.highest_sales_val AS highest_sales_val,
        l.lowest_category AS lowest_category,
        l.lowest_sales_val AS lowest_sales_val
